...
from dataset import *
from nasrgan_model import Generator, Discriminator
...
def train(args):
    ...
    while fine_epoch < args.fine_train_epoch:
        ...
        if fine_epoch % 500 ==0:
            #torch.save(generator.state_dict(), './model/NASRGAN_gene_%03d.pt'%fine_epoch)
            #torch.save(discriminator.state_dict(), './model/NASRGAN_discrim_%03d.pt'%fine_epoch)
            torch.save(generator.state_dict(), './model/NASRGAN_gene_%03d.pt'%fine_epoch)
            torch.save(discriminator.state_dict(), './model/NASRGAN_discrim_%03d.pt'%fine_epoch)
